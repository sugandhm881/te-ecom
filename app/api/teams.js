...
function slackToHtml(payload) {
    ...
    if (Array.isArray(payload.blocks) && payload.blocks.length) {
        for (const b of payload.blocks) {
            ...
            // Linked for the same reason the card Image carries a selectAction — tap to open full size.
            else if (b.type === 'image' && b.image_url) parts.push(`<a href="${b.image_url}"><img src="${b.image_url}" alt="${String(b.alt_text || '').replace(/"/g, '&quot;')}" style="max-width:100%;border-radius:8px"></a>`);
...
function slackToCardBody(payload, rich = false) {
    ...
    if (Array.isArray(payload.blocks) && payload.blocks.length) {
        for (const b of payload.blocks) {
            ...
            // `msteams.allowExpand` makes the image TAPPABLE IN TEAMS ITSELF — tap opens Teams' native
            // full-screen viewer with pinch-zoom. Without it a card image does nothing on tap (mobile
            // especially), so a dense report PNG was stuck at phone width. ⚠️ NOT a selectAction with
            // Action.OpenUrl: that was tried first and it opens the raw file in the BROWSER, which the
            // user explicitly rejected (2026-08-19) — the viewer must be Teams' own.
...
// THE WORKFLOWS WEBHOOK IS OFF (user, 2026-09-11: "on Teams report post come twice, once from Pravidhi and
...
async function postTeams(webhookUrl, payload, opts = {}) {
    ...
    // The Pravidhi bot is the delivery path: the card comes from the bot's own identity instead of the
    // generic "Workflows" sender, and the bot works in PRIVATE channels where an incoming webhook cannot
    // be created at all.
    //
    // The channel is resolved from the WEBHOOK URL rather than passed by every caller: each
    // TEAMS_WEBHOOK_<X> has a matching TEAMS_CHANNEL_<X>, so one lookup routes every existing report to
    // the bot with no call-site changes. The webhook URL is therefore still REQUIRED in .env — it is the
    // key that finds the channel — even though nothing is posted to it any more.
    ...
    if (channelId) {
        try {
            ...
            // ⚠️ A FAILED BOT POST HAS OFTEN ALREADY LANDED. sendToChannel waits 20 s for Teams to
