...
- `/api/teams/test?target=` — Teams webhook smoke test
  ...
  - **🔒 Shopify auto-hold for repeat COD orders (2026-07-20):** repeat COD orders are held on **Shopify** (a fulfillment-order hold) so support can phone-confirm before they ship. Chosen because the EasyEcom hold fails once the order is imported/manifested — a Shopify hold placed BEFORE EasyEcom imports keeps the order OUT of EasyEcom entirely (verified live). **Same "repeat" criteria as the Call Queue Repeat tab** (`findRepeatCandidates()` in `support_console.js`, shared): (1) COD, (2) still pre-pickup (bucket `order_to_dispatch`, the only holdable state), (3) order value > ₹1500, (4) ≥1 of the customer's last 3 PRIOR orders not delivered (RTO/undelivered/in-transit count; cancelled doesn't). In-transit/not-delivered orders are NOT listed as rows — they're the customer's history, seen via the order-detail click. **Service** `app/api/shopify_hold.js`: 2-step Shopify REST (`GET /orders/{id}/fulfillment_orders.json` → `POST /fulfillment_orders/{fo}/hold.json` `{fulfillment_hold:{reason:'other',reason_notes}}`; `release_hold.json` to undo). **State** = marks in `order_marks_ecom`: `shopify_hold` (held) / `shopify_hold_released` (human released → never auto-re-hold) / `shopify_hold_failed` (attempt failed → manual Hold button). **Triggers:** (1) instant — Shopify `orders/create` webhook `POST /api/webhook/shopify-order` (HMAC via `SHOPIFY_WEBHOOK_SECRET`, holds before EE can import); (2) backstop — `*/5` cron `SHOPIFY_AUTOHOLD_ENABLED=true`. **UI:** Call Queue → Repeat tab shows 🔒 On-hold badge + **Release** (post-confirmation) + **Hold** (manual/after a failure) via `POST /api/support/shopify-hold|shopify-unhold`. `SHOPIFY_TOKEN` already carries `write_merchant_managed_fulfillment_orders` + `write_assigned_fulfillment_orders`. **Setup left:** register the `orders/create` webhook → `https://dashboard.theelement.skin:8443/api/webhook/shopify-order` and set `SHOPIFY_WEBHOOK_SECRET`; flip `SHOPIFY_AUTOHOLD_ENABLED=true`.
